...
export default function Map({ activeIssue }) {
  ...
  const [prevActiveIssue, setPrevActiveIssue] = useState(activeIssue);

  if (activeIssue !== prevActiveIssue) { //reset state when user selects a different issue (replaces useEffect)
    setPrevActiveIssue(activeIssue);
  ...
  const handleCountryClick = async (geo) => {
    ...
    try {
      ...
      const response = await fetch(`/api/issues/${formattedIssue}/countries/${countryCode}`);
