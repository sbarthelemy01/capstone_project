...
# Capstone Step 6 - Build and Implementation

To access this app in your code editor, run the `git clone` command with the web url added at the end.
In the terminal, make sure you are in the infomap-next directory before running `npm install` to install the packages and dependencies. After that, enter `npm run dev` in your terminal to start the application. The terminal will provide a local host link which is usually http://localhost:3000. Open up this link in the browser.
The user should have access to log in or register their account so that they can navigate and save their searches to a favorites list. Here is a [demo](https://drive.google.com/file/d/15Msh__pa6dBCzdB6i685qDyAxoAxpKvB/view?usp=drive_link) showing how users can log in/register, switch between global issues, and add or remove countries (with the selected global issue) from their favorites list.
